...
import 'package:mindcoach/View/auth/auth_di.dart';
import 'package:mindcoach/View/auth/domain/social_login_provider.dart' show SocialLoginProvider;
 ...
 Future<void> login(SocialLoginProvider provider,BuildContext context) async {
    ...
    try {
      ...
      if (provider == SocialLoginProvider.facebook) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Facebook Login is not active now")));
      }else{
        
     UserModel? userModel = await ref?.read(AllProviders.authProvider.notifier).login(provider);
      if (userModel != null) {
       ...
       ref?.read(AllProviders.userProvider.notifier).setUserModel(userModel);
        ...
        if(userModel.credential == "guest"){
             await navigatorKey.currentState?.pushNamedAndRemoveUntil(PageRoutes.navbar, (a)=>false);
        }else{
