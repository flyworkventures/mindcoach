...
const Color _kLiveText = Color(0xFF111111);

class AppointmentVideoCallScreen extends ConsumerStatefulWidget {
  final AppointmentInfo appointment;

  const AppointmentVideoCallScreen({
    super.key,
    required this.appointment,
  });

  @override
  ConsumerState<AppointmentVideoCallScreen> createState() =>
      _AppointmentVideoCallScreenState();
}
...
class _AppointmentVideoCallScreenState
    extends ConsumerState<AppointmentVideoCallScreen> {
  ...
  void initState() {
    ...
    _initializeServices();
    _loadConsultantAndModel();
    _loadAnimations();
  }

  /// Randevu zamanını kontrol et ve başlat
  void _checkAppointmentTime() {
    final appointmentDateTime = widget.appointment.appointmentDateTime;
    if (appointmentDateTime == null) {
      _showErrorAndExit('Randevu tarihi bulunamadı');
      return;
    }
    ...
    if (timeDifference.inMinutes < -5) {
      _showErrorAndExit('Randevu zamanı geçmiş');
      return;
    }

    // Randevu henüz gelmemişse bekle
    if (timeDifference.inMinutes > 5) {
      _showErrorAndExit('Randevu zamanı henüz gelmedi');
      return;
    }
  ...
  /// Randevuyu başlat ve 30 dakika timer'ı başlat
  void _startAppointment() {
    ...
    _appointmentTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      final elapsed = DateTime.now().difference(_appointmentStartTime!);
      final totalRemainingSeconds = (30 * 60) - elapsed.inSeconds;

      if (totalRemainingSeconds <= 0) {
        // 30 dakika doldu, randevuyu bitir
        timer.cancel();
        _endAppointment();
        return;
      }
      ...
      setState(() {
        _remainingMinutes = totalRemainingSeconds ~/ 60;
        _remainingSeconds = totalRemainingSeconds % 60;
      });
    });
  }

  /// Randevuyu bitir ve geri dön
  void _endAppointment() {
  ...
  /// Consultant ve 3D model bilgilerini yükle
  Future<void> _loadConsultantAndModel() async {
    final consultantId = widget.appointment.consultantId;
    if (consultantId == null) {
      _showErrorAndExit('Consultant bulunamadı');
      return;
    }
    ...
    try {
      ...
      if (consultants == null || consultants.isEmpty) {
        _showErrorAndExit('Consultant listesi yüklenemedi');
        return;
      }
      ...
      setState(() {
        ...
        _threeDUrl = consultant.url3d;
      });
    } catch (e) {
      log('❌ Consultant yükleme hatası: $e');
      _showErrorAndExit('Consultant yüklenemedi');
    }
  }
  ...
  Future<void> _initializeServices() async {
    ...
    _audioPlayer!.playerStateStream.listen((state) {
      if (state.processingState == ProcessingState.completed) {
        ...
        log('✅ [APPOINTMENT-VIDEO-CALL] AI sesi bitti');
      }
    });
  ...
  Future<void> _loadAnimations() async {
    try {
      ...
      log('⚠️ Animasyon yükleme hatası: $e');
    }
  }

  Future<void> _startRecording() async {
    if (_isRecording || _audioRecorder == null || !_isAppointmentActive) return;
    ...
    try {
      ...
      _recordingPath = '${tempDir.path}/appointment_call_$timestamp.m4a';
  ...
  Future<void> _stopRecordingAndSend() async {
    ...
    try {
      ...
      final response = await _streamCallRepo!.sendStreamCallAudio(
        consultantId: _consultantModel!.id,
        audioFile: file,
      );
      ...
      if (response != null && response['success'] == true) {
        ...
        if (aiVoiceURL != null && aiVoiceURL.isNotEmpty) {
          ...
          log('⚠️ [APPOINTMENT-VIDEO-CALL] aiVoiceURL boş');
          if (mounted) {
            setState(() {
        ...
        throw Exception(
            'Audio gönderilemedi: ${response?['error'] ?? 'Bilinmeyen hata'}');
      }
    } catch (e) {
      log('❌ [APPOINTMENT-STREAM-CALL] Gönderme hatası: $e');
      setState(() {
        _status = 'Hata: $e';
        _isRecording = false;
        _isGlowing = false;
  ...
  Future<void> _playAIResponse(String aiVoiceURL) async {
    if (_audioPlayer == null || !_isAppointmentActive) return;
  ...
  void _startHeadActionAnimation() {
    try {
      final headActionAnimation = _availableAnimations.firstWhere(
        (anim) =>
            anim.toLowerCase().contains('headaction') ||
            anim.toLowerCase().contains('head_action'),
        orElse: () =>
            _availableAnimations.isNotEmpty ? _availableAnimations.first : '',
      );
      ...
      log('⚠️ Animasyon başlatma hatası: $e');
    }
  }
  ...
  void _stopHeadActionAnimation() {
    try {
      ...
      log('⚠️ Animasyon durdurma hatası: $e');
    }
  }
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              _buildAppointmentTimer(context),
          ...
          Padding(
            ...
            child: IconButton(
              ...
              icon: SvgPicture.asset(
                ...
                width: 12.w,
                colorFilter:
                    const ColorFilter.mode(_kLiveText, BlendMode.srcIn),
              ),
            ),
          ),
          Expanded(
  ...
  Widget _buildAppointmentTimer(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF2BD383).withOpacity(0.1),
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(width: 8.w),
          Text(
            'Kalan Süre: ${_remainingMinutes.toString().padLeft(2, '0')}:${_remainingSeconds.toString().padLeft(2, '0')}',
            style: GoogleFonts.quicksand(
              fontSize: 14.w,
  ...
  Widget _buildVideoCard(BuildContext context) {
    ...
    if (_threeDUrl == null || _threeDUrl!.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
  ...
  Widget _buildMicrophoneButton(BuildContext context) {
    return GestureDetector(
      ...
      onTap: () {
        if (!_isAppointmentActive) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Randevu süresi doldu')),
          );
          return;
        }
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: _isAppointmentActive
              ? (_isRecording ? Colors.red : const Color(0xFF2BD383))
              : Colors.grey,
          boxShadow: _isGlowing && _isAppointmentActive
              ? [
                  BoxShadow(
