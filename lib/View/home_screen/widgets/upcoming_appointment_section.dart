...
class UpcomingAppointmentSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Randevular yüklenene kadar bekle
    if (appointmentsState.appointments.isEmpty) {
      // Randevular henüz yükleniyor, loading göster veya gizle
...
class _AppointmentCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    bool isPassed30Minutes(DateTime givenTime) {
  final now = DateTime.now();
  return now.isAfter(givenTime.add(const Duration(minutes: 60)));
}
    ...
    String appointmentDate = "${date.day}.${date.month}.${date.year}";
    String nowDate = "${now.day}.${now.month}.${now.year}";

    return GestureDetector(
      onTap: () {
        if (ref.read(specialistsProvider).specialists?.isNotEmpty == true ) {
          if (appointmentDate == nowDate && !isPassed30Minutes(date)) {
             ref.read(conversationsProvider.notifier).startVideoCall(ref.read(specialistsProvider).specialists!.where((a)=>a.id == info.consultantId!).first,date);
                              Navigator.pushNamed(context, PageRoutes.videoCall);
          } else {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Randevu tarihiniz yaklaşmadı")));
          }
